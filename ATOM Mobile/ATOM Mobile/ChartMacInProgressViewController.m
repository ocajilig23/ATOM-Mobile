//
//  ChartMacInProgressViewController.m
//  ATOM Mobile
//
...
@interface ChartMacInProgressViewController ()

@end

@implementation ChartMacInProgressViewController
{
...
- (void)reloadData
{
    ...
    NSArray * categories = [defaults objectForKey:@"MachineYaxis"];
    NSData *gdata = [defaults objectForKey:@"MachineData"];
    NSSet *gdataset = [NSKeyedUnarchiver unarchiveObjectWithData:gdata];
    
    TKChartStackInfo *stackInfo = nil;
    
    stackInfo = [[TKChartStackInfo alloc] initWithID:@(1) withStackMode:TKChartStackModeStack];
    
    for(id item in gdataset){
    ...
    yAxis.range = [TKRange rangeWithMinimum:@0 andMaximum:[defaults objectForKey:@"MachineYaxisMax"]];
    yAxis.majorTickInterval = [defaults objectForKey:@"MachineYinterval"];
    _chart.yAxis = yAxis;
    _chart.title.text = [defaults objectForKey:@"MachineGraphTitle"];
    [_chart reloadData];
}
